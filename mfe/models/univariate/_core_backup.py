...
@jit(float64[:](float64[:], float64, float64, float64[:], float64), 
     nopython=True, cache=True)
def igarch_recursion(data, omega, beta, sigma2, backcast):
    """Compute IGARCH(1,1) conditional variances using Numba acceleration.
    
    This function implements the core recursion for the IGARCH(1,1) model:
    σ²_t = ω + (1 - β) * r²_{t-1} + β * σ²_{t-1}
    
    The IGARCH model is a special case of GARCH where alpha + beta = 1,
    which implies a unit root in the variance process (integrated variance).
    
    Args:
        data: Input data (typically residuals)
        omega: Constant term in variance equation
        beta: GARCH parameter (alpha = 1 - beta implicitly)
        sigma2: Pre-allocated array for conditional variances
        backcast: Value to use for initializing the variance process
    ...
    # Compute conditional variances recursively
    # In IGARCH, alpha = 1 - beta to ensure alpha + beta = 1
    alpha = 1.0 - beta
...
@jit(float64[:](float64, float64, float64, int64), 
     nopython=True, cache=True)
def igarch_forecast(omega, beta, last_variance, steps):
    """Generate analytic forecasts for IGARCH(1,1) model using Numba acceleration.
    
    In IGARCH models, the forecasts do not converge to an unconditional variance
    since the process has a unit root. Instead, the forecasts grow linearly with
    the forecast horizon if omega > 0.
...
@jit(float64[:](float64[:], float64, float64, float64, float64, float64[:], float64), 
     nopython=True, cache=True)
def agarch_recursion(data, omega, alpha, gamma, beta, sigma2, backcast):
    """Compute Asymmetric GARCH (AGARCH) conditional variances using Numba acceleration.
    
    This function implements the core recursion for the AGARCH(1,1) model:
    σ²_t = ω + α * (ε_{t-1} - γ)² + β * σ²_{t-1}
    
    The AGARCH model allows for asymmetric responses to positive and negative shocks
    by shifting the squared residuals by the asymmetry parameter gamma.
    ...
    # Compute conditional variances recursively
    for t in range(1, T):
        # Compute shifted squared residual: (ε_{t-1} - γ)²
        shifted_resid_squared = (data[t-1] - gamma) ** 2
        
        # Update variance: σ²_t = ω + α * (ε_{t-1} - γ)² + β * σ²_{t-1}
        sigma2[t] = omega + alpha * shifted_resid_squared + beta * sigma2[t-1]
...
@jit(float64[:](float64, float64, float64, float64, float64, int64), 
     nopython=True, cache=True)
def agarch_forecast(omega, alpha, gamma, beta, last_variance, steps):
    """Generate analytic forecasts for AGARCH(1,1) model using Numba acceleration.
    ...
    # Compute adjustment term for the asymmetry
    # E[(ε_t - γ)²] = E[ε_t²] - 2γE[ε_t] + γ²
    # For standardized residuals, E[ε_t] = 0 and E[ε_t²] = 1
    asymmetry_adjustment = alpha * gamma * gamma
    
    # First step forecast
    forecasts[0] = omega + alpha * (1 + gamma * gamma) + beta * last_variance
    
    # Multi-step forecasts
    for h in range(1, steps):
        forecasts[h] = omega + asymmetry_adjustment + persistence * forecasts[h-1]
...
def egarch_recursion(data, omega, alpha, gamma, beta, log_sigma2, backcast):
    ...
    Args:
        ...
        alpha: ARCH parameter for the magnitude effect
        gamma: Asymmetry parameter for the sign effect
        beta: GARCH parameter for persistence
...
def egarch_p_q_recursion(data, omega, alpha, gamma, beta, log_sigma2, backcast):
    ...
    log(σ²_t) = ω + Σ(α_i * (|z_{t-i}| - E[|z|]) + Σ(β_j * log(σ²_{t-j}))
...
def simulate_egarch_path(innovations, omega, alpha, gamma, beta, log_sigma2, initial_log_variance):
    """Simulate a path from an EGARCH(1,1) model using Numba acceleration.
    
    Args:
        innovations: Array of random innovations
        omega: Constant term in log-variance equation
        alpha: ARCH parameter for the magnitude effect
        gamma: Asymmetry parameter for the sign effect
        beta: GARCH parameter for persistence
        log_sigma2: Pre-allocated array for conditional log-variances
        initial_log_variance: Initial value for the log-variance process
    ...
    # Compute standardized innovation for first period
    z = innovations[0]
    
    # Simulate the process
    for t in range(1, T):
        # Update log-variance
        log_sigma2[t] = omega + alpha * (np.abs(z) - np.sqrt(2/np.pi)) + gamma * z + beta * log_sigma2[t-1]
        
        # Compute standardized innovation for next period
        if t < T - 1:
            z = innovations[t]
...
def heavy_recursion(data, omega, beta, sigma2, backcast):
    ...
    This function implements the core recursion for the HEAVY model:
    σ²_t = ω + β * σ²_{t-1} + (1 - β) * r²_{t-1}
    
    The HEAVY (High-frequency-based Volatility) model uses realized measures
    of volatility to improve volatility forecasting.
    
    Args:
        data: Input data (typically realized measures of volatility)
        omega: Constant term in variance equation
    ...
    # Compute conditional variances recursively
    alpha = 1.0 - beta  # In HEAVY model, alpha + beta = 1 is often imposed
    
    for t in range(1, T):
        sigma2[t] = omega + alpha * data[t-1] + beta * sigma2[t-1]
...
@jit(float64[:](float64, float64, float64, int64), 
     nopython=True, cache=True)
def heavy_forecast(omega, beta, last_variance, steps):
    ...
    # Compute unconditional variance
    unconditional_variance = omega / alpha if alpha > 0 else last_variance
    
    # First step forecast
    forecasts[0] = omega + beta * last_variance + alpha * last_variance
    
    # Multi-step forecasts
    for h in range(1, steps):
        forecasts[h] = omega + beta * forecasts[h-1] + alpha * unconditional_variance
